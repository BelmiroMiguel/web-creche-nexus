...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const slideInfos = [
    // Textos correspondentes aos slides
  ...
  function showSlide(n) {
    ...
    slideIndex = n;
    if (slideIndex >= bgSlides.length) {
      slideIndex = 0;
    }
    if (slideIndex < 0) {
      slideIndex = bgSlides.length - 1;
    }
  ...
  document.querySelectorAll(".toggle-senha").forEach((btn) => {
    btn.addEventListener("click", function () {
      const inputSenha = this.closest(".campo-form-login").querySelector(
  ...
  function mostrarArea(areaParaMostrar, areaParaEsconder) {
    if (areaParaMostrar && areaParaEsconder) {
      ...
      setTimeout(() => {
        // Garante que a animação de 'out' possa começar
        areaParaMostrar.classList.add("ativa");
        // Foco no primeiro input da nova área
        const primeiroInput = areaParaMostrar.querySelector(
  ...
  function validarCampo(input) {
    ...
    const msgErroEl = campoForm
      ? campoForm.querySelector(".mensagem-erro")
      : null;
    let valido = true;
    let msgErro = "";
  ...
  document
    .querySelectorAll(".campo-form-login input[required]")
    .forEach((input) => {
      ...
      AppService.postData(
        ...
        { email: usuario, senha: formLogin.senha.value },
        {
          ...
          onError: (error) => {
            ...
            const msgErroSenha = formLogin.senha
              .closest(".campo-form-login")
              .querySelector(".mensagem-erro");
            if (msgErroSenha)
              msgErroSenha.textContent = "Credenciais inválidas.";
          },
        }
      );
    });
  ...
  if (formRecuperarSenha) {
    formRecuperarSenha.addEventListener("submit", (e) => {
      ...
      const btnRecuperar = formRecuperarSenha.querySelector(
        'button[type="submit"]'
      );
  ...
  function aplicarTemaLogin(tema) {
    ...
    if (botaoModoTemaLogin) {
      ...
      botaoModoTemaLogin.setAttribute(
        "aria-label",
        tema === "modo-escuro"
  ...
  if (botaoModoTemaLogin) {
    botaoModoTemaLogin.addEventListener("click", () => {
      const temaAtual = document.body.classList.contains("modo-escuro")
        ? "modo-claro"
        : "modo-escuro";
  ...
  } else if (temaGlobalApp !== null) {
    // Se tema do login não salvo, mas o do app sim
