...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const botaoDropdownUsuario = document.getElementById(
    "botao-dropdown-usuario"
  ); // Atualizado
  ...
  const appItensMenu = appLauncherMenu
    ? appLauncherMenu.querySelectorAll(".app-item")
    : [];

  // Conteúdo Principal
  const conteudoPrincipalApp = document.getElementById(
    "conteudo-principal-app"
  );
  const todasSecoes = conteudoPrincipalApp
    ? Array.from(conteudoPrincipalApp.querySelectorAll(".secao-app"))
    : []; // Convertido para Array para usar .find
  ...
  function inicializarNexusOS() {
    ...
    if (conteudoPrincipalApp && todasSecoes.length > 0) {
      ...
      if (secaoDoHash) {
        ...
        // Hash existe mas não é uma seção válida
      ...
      console.warn(
        "Conteúdo principal ou seções não encontradas. Navegação limitada."
      );
    }

    if (botaoDropdownUsuario && menuUsuario) configurarDropdownUsuario();
    else console.warn("Dropdown do usuário não encontrado.");
    if (botaoToggleLauncher && appLauncherMenu) configurarAppLauncher();
    else console.warn("App Launcher não encontrado.");
  ...
  function configurarModoTema() {
    const aplicarTema = () => {
      ...
      if (botaoModoTema) {
        const icone = modoEscuroAtivo
          ? '<i class="fas fa-moon"></i>'
          : '<i class="fas fa-sun"></i>';
        const label = modoEscuroAtivo
    ...
    if (botaoModoTema) {
      botaoModoTema.addEventListener("click", () => {
        ...
        document.dispatchEvent(
          new CustomEvent("temaAlterado", {
            detail: { modoEscuro: modoEscuroAtivo },
          })
        );
  ...
  function atualizarDataHora() {
    ...
    if (dataAtualEl)
      dataAtualEl.textContent = agora.toLocaleDateString("pt-BR", opcoesData);
    if (horaAtualEl)
      horaAtualEl.textContent = agora.toLocaleTimeString("pt-BR", opcoesHora);
  }
  ...
  function mostrarSecao(idSecao, adicionarAoHistorico = true) {
    ...
    if (!secaoAlvo) {
      console.warn(
        `Seção inválida: "${idSecao}". Redirecionando para o dashboard.`
      );
      if (idSecao !== "dashboard" && document.getElementById("dashboard")) {
        ...
        console.error(
          "Seção 'dashboard' também não encontrada. Verifique o HTML."
        );
        if (todasSecoes.length > 0) {
          // Mostra a primeira seção disponível como fallback final
    ...
    const nomeSecaoFormatado =
      idSecao.charAt(0).toUpperCase() + idSecao.slice(1).replace(/-/g, " ");
    ...
    if (adicionarAoHistorico && window.history && window.history.pushState) {
      ...
      // Apenas atualiza o título se não estiver adicionando ao histórico (ex: popstate)
      document.title = novoTitulo;
    }
  }
  ...
  function configurarNavegacaoSecoes() {
    document.querySelectorAll("[data-secao]").forEach((link) => {
      link.addEventListener("click", (e) => {
        ...
        if (idSecao && idSecao !== secaoAtivaAtual) {
          // Só navega se for diferente da atual
        ...
        } else if (idSecao && idSecao === secaoAtivaAtual) {
          // Se clicou na seção ativa, fecha menus
          if (appLauncherMenu?.classList.contains("aberto"))
            toggleLauncherState(true);
          if (botaoDropdownUsuario?.getAttribute("aria-expanded") === "true") {
            botaoDropdownUsuario.setAttribute("aria-expanded", "false");
    ...
    window.addEventListener("popstate", (event) => {
      ...
      if (
        event.state &&
        event.state.secao &&
        document.getElementById(event.state.secao)
      ) {
  ...
  function configurarDropdownUsuario() {
    botaoDropdownUsuario.addEventListener("click", (e) => {
      e.stopPropagation();
      const isExpanded =
        botaoDropdownUsuario.getAttribute("aria-expanded") === "true";
    ...
    document.addEventListener("click", (e) => {
      if (
        ...
        // Verifica se menuUsuario existe
        botaoDropdownUsuario.setAttribute("aria-expanded", "false");
      }
    });
  ...
  const toggleLauncherState = (forceClose = false) => {
    ...
    if (forceClose || isOpen) {
      ...
      const firstFocusableInLauncher =
        launcherBotaoFechar || appLauncherMenu.querySelector(".app-item");
  ...
  function configurarAppLauncher() {
    ...
    if (launcherBotaoFechar) {
      launcherBotaoFechar.addEventListener("click", () =>
        toggleLauncherState(true)
      );
    }
    ...
    document.addEventListener("click", (event) => {
      if (
        appLauncherMenu.classList.contains("aberto") &&
  ...
  function ativarItemMenu(idSecao) {
    ...
    appItensMenu.forEach((item) => {
      ...
      item.setAttribute(
        "aria-current",
        item.dataset.secao === idSecao ? "page" : "false"
      );
    });
  }
  ...
  function configurarModaisGlobais() {
    document.addEventListener("click", (e) => {
      ...
      const fecharBtn = e.target.closest(
        "[data-modal-fechar], .modal-botao-fechar"
      );
      ...
      if (abrirBtn) {
        ...
        e.preventDefault();
        const modalId =
          fecharBtn.dataset.modalId || fecharBtn.closest(".modal-overlay")?.id;
  ...
  window.abrirModalNexus = (modalId) => {
    ...
    if (scrollbarWidth > 0) {
      // Aplica padding apenas se houver scrollbar
      body.style.paddingRight = scrollbarWidth + "px";
      if (
        cabecalhoApp &&
        getComputedStyle(cabecalhoApp).position === "sticky"
      ) {
        // Ajusta header sticky
  ...
  window.mostrarToast = function (mensagem, tipo = "info", duracao = 5000) {
    ...
    if (!toastContainer) {
      console.warn(
        "Toast container #toast-container não encontrado. Usando alert()."
      );
    ...
    toast.setAttribute(
      "role",
      tipo === "erro" || tipo === "aviso" ? "alert" : "status"
    );
    ...
    toast
      .querySelector(".toast-botao-fechar")
  ...
  function configurarInteracoesFormulariosEspecificas() {
    ...
    const selectFrequentouCreche = document.getElementById(
      "frequentou-outra-creche-matricula"
    );
    const campoCrecheAnterior = document.getElementById(
      "campo-historico-creche-anterior"
    );
    ...
    const selectTipoDestinatario = document.getElementById(
      "tipo-destinatario-comunicado"
    );
    const containerDestEspecifico = document.getElementById(
      "container-destinatario-especifico"
    );
    const selectTurmaComunicado = document.getElementById(
      "select-turma-especifica-comunicado"
    ); // Corrigido ID
    const selectAlunoComunicado = document.getElementById(
      "select-aluno-especifico-comunicado"
    ); // Corrigido ID

    if (
      selectTipoDestinatario &&
      containerDestEspecifico &&
      selectTurmaComunicado &&
      selectAlunoComunicado
    ) {
    ...
    function setupImagePreviewAndClick(inputId, previewId, buttonId) {
      ...
      if (button && input) {
        // Associa o botão ao clique do input
    ...
    setupImagePreviewAndClick(
      "config-logo-creche-input",
      "preview-logo-creche",
      "config-logo-creche-input"
    ); // Assumindo que o label com ID é o trigger
    setupImagePreviewAndClick(
      "upload-avatar-perfil-input",
  ...
  try {
    ...
    if (loadingOverlay) {
      // Garante que o loading overlay seja removido em caso de erro fatal
      loadingOverlay.setAttribute("hidden", "");
    }
    // Poderia mostrar uma mensagem de erro amigável para o usuário aqui
    const errorDiv = document.createElement("div");
    errorDiv.style.cssText =
      "position:fixed;top:0;left:0;width:100%;padding:20px;background:red;color:white;text-align:center;z-index:10001;";
    errorDiv.textContent =
      "Ocorreu um erro ao carregar o sistema. Por favor, tente recarregar a página ou contate o suporte.";
